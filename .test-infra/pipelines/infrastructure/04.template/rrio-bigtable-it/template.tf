  ...
  provisioner "local-exec" {
    command = <<EOF
      gcloud dataflow flex-template build \
        ...
        --metadata-file=${path.module}/template-parameters.json \
        --sdk-language=JAVA \
        --flex-template-base-image=JAVA11 \
        --jar=${local.beam_root}/${var.gradle_project_dir}/build/libs/${var.template_jar_name} \
        --env=FLEX_TEMPLATE_JAVA_MAIN_CLASS=${var.template_main_class} \
        --network=${data.google_compute_network.default.name} \
        --subnetwork=regions/${var.region}/subnetworks/${data.google_compute_subnetwork.default.name} \
        --disable-public-ips \
        --temp-location=gs://dataflow-staging-${var.region}-${data.google_project.default.number}/temp \
